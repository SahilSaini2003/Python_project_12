...
while(m == 1):
    ...
    if(a == 1):
        mdb = s.connect(host="localhost", user="root", port="6603",
                        password="root", database="admin", auth_plugin='mysql_native_password')
        cur = mdb.cursor()
        print("\t\t\t$ Lets Check Weather You Are Admin or Not $")
        ad = input("\t\t\t\t Enter Admin Name:-")
        pa = input("\t\t\t\t Enter Password:-")
        q = "select * from Admin_list where Admin='{}' AND Password='{}'".format(
            ad, pa)
        cur.execute(q)
        rec = cur.fetchall()
        if(cur.rowcount == 0):
            ...
            while(q == 1):
                ...
                b = int(
                    input("\t\t\tWaana Enter the Numeric Value Of Your Choice:-"))
                if(b == 1):
                    w = 1
                    while(w == 1):
                        ...
                        if(c == 1):
                            ...
                            sq = "insert into Admin_list values('{}','{}')".format(
                                adn, Pass)
                            cur.execute(sq)
                            mdb.commit()
                            sq = "select * from Admin_list"
                            ...
                            add = input(
                                "\t\t\t        Enter Admin you want to Delete:-")
                            sq = "select * from Admin_list where Admin='{}'".format(
                                add)
                            cur.execute(sq)
                            rec = cur.fetchall()
                            if(cur.rowcount == 0):
                                ...
                                print("\t\t\t           OOPS! Admin not found")
                                print(
                                    "\t\t< Weather You entered Wrong Name or Admin Not Present >")
                            else:
                                print("\t\t\t\t `Admin Found`")
                                sq = "Delete from Admin_list where Admin='{}'".format(
                                    add)
                                cur.execute(sq)
                                mdb.commit()
                                sq = "select * from Admin_list"
                            ...
                            if(cur.rowcount == 0):
                                ...
                                sq = "update Admin_list set Password='{}' where Admin='{}'".format(
                                    pa, add)
                                cur.execute(sq)
                                mdb.commit()
                                sq = "select * from Admin_list where Admin='{}'".format(
                            ...
                            q = 0
                            print(
                                "\t\t    Please enter Correct Numeric value of your choice")
                        if(w == 0):
                            cho = input(
                    ...
                    while(w == 1):
                        ...
                        u = int(
                            input("\t\t\tWaana Enter the Numeric Value Of Your Choice:-"))
                        if(u == 1):
                            dfu = pd.read_excel(r'User.xlsx')
                            print(dfu)
                            w = 0
                            q = 0
                        elif(u == 2):
                            usr = input(
                                "\t\t\t    *Enter User Name that you want to Ban:-")
                            df2 = pd.read_excel(r'User.xlsx')
                            df3 = df2[df2['User Name'] == usr]
                            if(df3.empty):
                                print("\t\t\t           OOPS! User not found")
                                print(
                                    "\t\t< Weather You entered Wrong Name or User Not Present >")
                                w = 1
                                q = 0
                            else:
                                w = 0
                                ...
                                if(s == "y"):
                                    data = (
                                        df2[df2['User Name'] == usr].index.values)
                                    df2 = df2.drop(data)
                                    print("\t\t\t\t *Banned Successfully*")
                                    df2.to_excel(r'User.xlsx', index=False)
                                else:
                                    q = 0
                                    w = 0
                    ...
                    if(gh == 1):
                        plt.plot(
                            grf['Month'], grf['Profit Amount per Transection'], marker='*')
                        ...
                        plt.bar(grf['Month'],
                                grf['Profit Amount per Transection'])
                        plt.title("Profit made per Month", fontsize=20)
                        plt.xlabel('Month', fontsize=10)
                        plt.ylabel('Profit', fontsize=10)
                        plt.show()
        ...
        while(i == 1):
            if(b == 1):
                ...
                df2 = pd.read_excel(r'User.xlsx')
                data = {"User Id": [usi], "User Name": [usn], "Password": [
                    pas], "Mobile No.": [mo], "D.O.B": [dob], "User login Date": [lod]}
                df3 = pd.DataFrame(data)
                df4 = df2.append(df3)
                df4.to_excel(r'User.xlsx', index=False)
                ...
                while(w == 1):
                    ...
                    if(c == 1):
                        ...
                        udf2 = udf[(udf['User Id'] == usi) &
                                   (udf['Password'] == pas)]
                        if(udf2.empty):
                            print("\t\t\t           OOPS! User not found")
                            print(
                                "\t\t< Weather You entered Wrong Name/Password or User Not Present >")
                            i = 0
                            w = 1
                        else:
                            i = 0
        ...
        while(w == 1):
            if(uc == 1):
                ...
                if(uc1 == 1):
                    ...
                    if(udf1.empty):
                        ...
                        print("\t\t\t           OOPS! User not found")
                        print(
                            "\t\t< Weather You entered Wrong User Name/D.O.B or User Not Present >")
                    else:
                        w = 0
                        pas = input("\t\t\t\t Enter New Password:-")
                        d = (udf[udf['User Name'] == usn].index.values)
                        udf.loc[d, ["Password"]] = pas
                        udf.to_excel(r'User.xlsx', index=False)
                    ...
                    if(udf2.empty):
                        ...
                        print("\t\t\t           OOPS! User not found")
                        print(
                            "\t\t< Weather You entered Wrong User Name/Password or User Not Present >")
                    else:
                        w = 0
                        usi = input(
                            "\t\t\t        Enter New User Id/Email-Address:-")
                        d = (udf[udf['User Name'] == usn].index.values)
                    ...
                    if(uc1 == 1):
                        ...
                        if(dr.empty):
                            ...
                            if(re == 1):
                                ...
                                print("\t\t\t\t       # 3.Airtel #")
                                op = int(
                                    input("\t\t\tWaana Enter the Numeric Value of Operator:-"))
                                if(op == 1):
                                    op = "Jio"
                                    ...
                                    print(jp)
                                    pa = int(
                                        input("\t\t\t     Enter the Amount of Choosen Plan:-"))
                                    d = (
                                        jp[jp['Cost in Rs.'] == pa].index.values)
                                    print(jp.loc[d])
                                    print("\t\t\t\t     *Pay=", pa)
                                    ...
                                    if(pay == "y"):
                                        ...
                                        y = 1
                                        amg = pa*0.01
                                        qg = grf[(grf['Month'] == mont)]
                                        if(qg.empty):
                                            gr = {'Profit Amount per Transection': [
                                                amg], 'Month': [mont]}
                                            gf1 = pd.DataFrame(gr)
                                            grf = grf.append(gf1)
                                        else:
                                            qw = (
                                                grf[grf['Month'] == mont].index.values)
                                            nw = np.array(grf)
                                            nw1 = nw[qw, 0]
                                            amg1 = amg+nw1
                                            grf.loc[qw, [
                                                "Profit Amount per Transection"]] = amg1
                                        grf.to_csv('Graph.csv', index=False)
                                        at = uc1+"-"+op
                                        ...
                                        h2 = h1[0, 0]
                                        h3 = h2+1
                                        a = {"Transection ID": [h3], "User Name": [usn], "Transection Amount": [
                                            pa], "Amount Type": [at], "MobileNo./K.No./E-Mitra CID Code": [no], "Date": [dat]}
                                        hf1 = pd.DataFrame(a)
                                        hf2 = hf.append(hf1)
                                    ...
                                    print("\t\t\t\t     # 2.Postpaid #")
                                    pp = int(
                                        input("\t\t\tWaana Enter the Numeric Value Of Service:-"))
                                    if(pp == 1):
                                        print("\t\t\t\t || Plans available ||")
                                        ...
                                        am = int(
                                            input("\t\t\t\t Enter Amount to Pay in Rupees:-"))
                                        print("\t\t\t\t *Pay=", am)
                                        pay = input(
                                            "\t\t\t\t        yes/no(y/n):-")
                                        y = 0
                                        if(pay == "y"):
                                            ...
                                            h3 = h2+1
                                            a = {"Transection ID": [h3], "User Name": [usn], "Transection Amount": [
                                                am], "Amount Type": [at], "MobileNo./K.No./E-Mitra CID Code": [no], "Date": [dat]}
                                            hf1 = pd.DataFrame(a)
                                            hf2 = hf.append(hf1)
                                            hf2.to_csv(
                                                'History.csv', index=False)
                                ...
                                print("\t\t\t\t            # 3.Dish Tv #")
                                to = int(
                                    input("\t\t\tWaana Enter the Numeric Value of Choosen Operator:-"))
                                if(to == 1):
                                    op = "D2h Videocon"
                                    print(
                                        "\t\t\t    # 1.Pay Using Registered Mobile No.(10 digits) #")
                                    print(
                                        "\t\t\t           # 2.Pay Using Customer Id(7 Digits) #")
                                    to1 = int(
                                        input("\t\t\tWaana Enter the Numeric Value of Choice:-"))
                                    if(to1 == 1):
                                        no = int(
                                            input("\t\t\t       Enter Registered Mobile No.(10 digits):-"))
                                        print(
                                            "\t\t\t\t *Confirm Your Number:-", no)
                                        z = input(
                                            "\t\t\t\t      yes/no(y/n):-")
                                        if(z == "y"):
                                            y = 0
                                            am = int(
                                                input("\t\t\t\t Enter Amount in Rupee:-"))
                                            print("\t\t\t\t *Pay=", am)
                                            r = input(
                                                "\t\t\t\t      yes/no(y/n):-")
                                            if(r == "y"):
                                        ...
                                        no = int(
                                            input("\t\t\t\tEnter Customer Id(7 Digits):-"))
                                    ...
                                    op = "Tata Sky"
                                    no = int(
                                        input("\t\t\t\t Enter Customer Id(10 Digits):-"))
                                    print("\t\t\t\t *Confirm your Id:-", no)
                                    z = input("\t\t\t\t yes/no(y/n):-")
                                    if(z == "y"):
                                        y = 0
                                    ...
                                    op = "Dish Tv"
                                    no = int(
                                        input("\t\t\t\t Enter Customer Id(12 Digits):-"))
                                    print("\t\t\t\t *Confirm Your Id", no)
                                    z = input("y/n")
                                    if(z == "y"):
                                        y = 0
                        ...
                        if(dr.empty):
                            ...
                            if(bi == 1):
                                ...
                                if(ba == 1):
                                    ...
                                    print(
                                        "t\t    Please enter Correct Numeric value of your choice")
                                    q = 1
                            else:
                                print(
                                    "t\t    Please enter Correct Numeric value of your choice")
            ...
            if(y == 1):
                ...
                while(v == 1):
                    ...
                    print(
                        "\t\t\t         # 1.Already saved Credit Card or Debit card #")
                    ...
                    if(c == 1):
                        ...
                        df1 = df[(df['User Id'] == u) & (
                            df['Password'] == p) & (df['PIN'] == pi)]
                        if(df1.empty):
                            v = 1
                            print("\t\t\t           OOPS! not found")
                            print(
                                "\t\t< Weather You entered Wrong Details or Not Present >")
                        else:
                            v = 0
                            ...
                            if(ps == "y"):
                                ...
                                print("\t\t\t\t *Do You Want a Recepit*")
                                pe = input("\t\t\t\t   yes/no(y/n):-")
                                if(pe == "y"):
                                    print(
                                        "\t\t\t\t || Make Sure Printer Should Be connected ||")
                                    print("\t\t\t\t *Is Your Printer Connected ")
                                    pq = input("\t\t\t\t    yes/no(y/n):-")
                                    if(pq == "y"):
                                        rg == 1
                                        m = 0
                        ...
                        ca = int(
                            input("\t\t\t\t Enter Card no(16 Digits No.):-"))
                        va = input("\t\t\t\t Card Validity:-")
                        cv = int(input("\t\t\t\t Enter CVV No.:-"))
                        pin = int(
                            input("\t\t\t  Enter New Pin(Can't Be Changed in FUTURE):-"))
                        a = {"User Id": [us], "Password": [pa], "Card No.": [
                            ca], "Expiry/Validity No.": [va], "CVV No.": [cv], "PIN": [pin], "Debit/Credit Card": [dc]}
            ...
            if(rg == 1):
                ...
                if(uf1.empty):
                    ...
                    uf2 = uf[(uf['User Name'] == usn) &
                             (uf['Transection ID'] == us)]
                    if(uf2.empty):
            ...
            if(p == 1):
                du = input(
                    "\t\t\t Do You Want To do Anything Else yes/no(y/n):- ")
                if(du == "y"):
                    q = 1
                else:
                    print("~:~"*36)
